...
def plot_false_alarm_analysis():
    ...
    # Add value labels
    for bar, rate in zip(bars, fa_rates, strict=False):
        ax1.text(
    ...
    ax1.axhline(
        y=10, color="red", linestyle="--", linewidth=2, label="Clinical Acceptability Threshold"
    )
...
def plot_prediction_distribution(checkpoint):
    ...
    # Add threshold line
    ax.axvline(x=0.8, color="black", linestyle="--", linewidth=2, label="Threshold (0.8)")
