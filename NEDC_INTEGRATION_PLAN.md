...
— This is the canonical plan for NEDC integration. Keep `docs/technical/NEDC_INTEGRATION_PLAN.md` in sync or reference this document. —
...
## 🏗️ Target Architecture (Phase 1 now, Phase 2 later)

Phase 1 (now, required)
- Keep official NEDC v6.0.0 as source of truth.
- Own the integration pipeline to generate valid CSV_bi and run scorer.
- Build a conformance suite with golden outputs from the official scorer.

Phase 2 (later, optional)
- Implement native `seizure_evaluation` scorer(s) that match NEDC within tolerance.
- Gate switching default backend on conformance and clinical acceptance.
...
## 🧰 Tooling & CI Policy (already configured in repo)
...
- `tests/integration/test_native_taes.py` (Phase 2)
...
## 📈 Operating-Point Tuning Workflow (dev → freeze → eval)
...
Rollout
- Phase 1: Solidify integration and conformance fixtures; default backend = `nedc-binary`.
- Phase 2: Implement native TAES; run side-by-side; switch when gates pass.
...
|------|----------|-----------|------------------|
| 1 | 3–5 days | Integration & Conformance | Golden fixtures, parsing, CI skip, gating |
| 2 | 3–4 days | Clinical Tuning | Dev sweeps, recommendation.json, freeze plan |
| 3 | 2–3 days | Eval Run | Frozen params, single eval, metrics archived |
| 4 | 4–7 days | Native TAES (optional) | Python scorer, parity tests, backend toggle |

Critical path
1. Integration/golden fixtures → 2. Dev tuning → 3. Freeze → 4. Eval once.
