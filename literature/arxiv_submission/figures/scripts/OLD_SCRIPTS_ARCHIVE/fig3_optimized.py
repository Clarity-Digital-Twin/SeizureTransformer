...
def generate_fig3_optimized():
    ...
    fig, ax = plt.subplots(figsize=(7, 7), dpi=DPI_ARXIV)

    # Define vertical positions (top to bottom flow)
    input_x, input_y = 3.5, 6.0
    model_x, model_y = 3.5, 4.5
    scorer_y = 3.0
    result_y = 1.5
    ...
    ax.text(2.0, 5.3, 'Same data', ha='center', fontsize=9,
           style='italic', color='black', alpha=0.8)
    ax.text(5.8, 3.8, 'Identical predictions', ha='center', fontsize=9,  # Moved right to avoid arrows
           style='italic', color='black', alpha=0.8)
    ax.text(0.5, 2.25, 'Different\nscorers', ha='center', fontsize=9,  # Slightly left
           style='italic', color='black', alpha=0.8)
