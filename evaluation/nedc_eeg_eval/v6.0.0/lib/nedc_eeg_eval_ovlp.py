...
# file: $NEDC_NFC/class/python/nedc_eeg_eval_tools/nedc_eeg_eval_ovlp.py
#
# revision history:
#
...
def run(events_ref, events_hyp, mapping, nedc_ovlp, odir, rfile, fp):
    """
    function: run
    ...
    #
    if dbgl > ndt.BRIEF:
        print("%s (line: %s) %s: running ovlp scoring" %
    ...
    status = novlp.score(events_ref, events_hyp, mapping, rfile)
    if status == False:
        print("Error: %s (line: %s):%s: error during results" %
              (__FILE__, ndt.__LINE__, ndt.__NAME__))
        return False
    ...
    def score(self, all_events_ref, all_events_hyp, score_map, rfile):
        ...
        for key_ref, key_hyp in zip(all_events_ref, all_events_hyp, strict=False):
            ...
            self.rfile_d.write("%5d: %s" % (i, key_ref) +
                               nft.DELIM_NEWLINE)
            self.rfile_d.write("%5s  %s" % (nft.STRING_EMPTY, key_hyp) +
                               nft.DELIM_NEWLINE)
            self.rfile_d.write("  Ref: %s" % nft.DELIM_SPACE.join(refo) +
                               nft.DELIM_NEWLINE)
            self.rfile_d.write("  Hyp: %s" % nft.DELIM_SPACE.join(hypo) +
                               nft.DELIM_NEWLINE)
            self.rfile_d.write("%6s (%s %d  %s %d  %s %d  Total: %d)" %
                               (nft.STRING_EMPTY, "Hit:", hit, "Miss:", mis,
                                "False Alarms:", fal, hit + mis + fal) +
                               nft.DELIM_NEWLINE)
